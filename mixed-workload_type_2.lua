...
local function search_hotel() 
  ...
  local method = "GET"
  local path = "http://localhost:8080/function/hotels?inDate=" .. in_date_str .. 
...
local function recommend()
  ...
  local method = "GET"
  local path = "http://localhost:8080/function/recommendations?require=" .. req_param .. 
...
local function reserve()
  ...
  local method = "POST"
  local path = "http://localhost:8080/function/reservation?inDate=" .. in_date_str .. 
    "&outDate=" .. out_date_str .. "&lat=" .. tostring(lat) .. "&lon=" .. tostring(lon) ..
...
local function user_login()
  ...
  local method = "GET"
  local path = "http://localhost:8080/function/user?username=" .. user_name .. "&password=" .. password
  local headers = {}
  -- headers["Content-Type"] = "application/x-www-form-urlencoded"
  return wrk.format(method, path, headers, nil)
end
